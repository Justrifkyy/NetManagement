<x-app-layout>
    <div class="py-10 bg-slate-950 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <h2 class="text-3xl font-extrabold text-white mb-8 px-4 sm:px-0">Integrasi Layanan Pihak Ketiga</h2>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 px-4 sm:px-0">
                @forelse ($integrations as $integration)
                    <div class="bg-slate-900 rounded-xl shadow-sm border border-slate-800 p-6">
                        <h3 class="font-bold text-lg text-white mb-4">{{ $integration->service_name }}</h3>
                        <div class="space-y-2 mb-4">
                            <p class="text-sm text-slate-300"><strong>API Key:</strong> •••••••••••</p>
                            @if ($integration->webhook_url)
                                <p class="text-sm text-slate-300"><strong>Webhook:</strong> Terkonfigurasi</p>
                            @endif
                        ...
                        <div class="flex gap-2">
                            <button onclick="testConnection({{ $integration->id }})" class="flex-1 px-3 py-2 bg-amber-600 text-white text-sm rounded-lg hover:bg-amber-700">Test</button>
                ...
                @empty
                    <div class="col-span-full text-center py-12">
                        <p class="text-slate-400">Belum ada integrasi</p>
                    </div>
                @endforelse
            </div>

            <!-- Add New Integration -->
            <div class="mt-8 bg-slate-900 rounded-xl shadow-sm border border-slate-800 p-6 px-4 sm:px-0">
                <h3 class="text-xl font-bold text-white mb-4">Tambah Integrasi Baru</h3>
                
                <form action="{{ route('admin.integrations.store') }}" method="POST" class="space-y-4">
                    @csrf

                    <div>
                        <label class="block text-sm font-medium text-slate-300 mb-2">Nama Layanan <span class="text-red-600">*</span></label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-300 mb-2">API Key <span class="text-red-600">*</span></label>
                    ...
                    <div>
                        <label class="flex items-center">
                            ...
                            <span class="ml-2 text-sm font-medium text-slate-300">Aktifkan Integrasi</span>
                        </label>
                    </div>
