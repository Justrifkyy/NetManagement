    ...
    <div class="py-10 bg-slate-950 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-8 px-4 sm:px-0 flex flex-col md:flex-row justify-between items-start md:items-center">
                <div>
                    <h2 class="text-3xl font-extrabold text-white">Manajemen Pelanggan</h2>
                    <p class="text-slate-400 mt-1">Kelola status layanan, aktivasi, dan isolir pelanggan.</p>
                </div>
            </div>
            ...
            <div class="mb-6 px-4 sm:px-0">
                <form action="{{ route('admin.customers.index') }}" method="GET" class="flex gap-4">
                    <input type="text" name="search" placeholder="Cari pelanggan..." 
                        class="flex-1 px-4 py-2 border border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead class="bg-slate-950 border-b border-slate-800">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">Kode Pelanggan</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">Nama</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">No. HP</th>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @forelse ($customers as $customer)
                                <tr class="hover:bg-slate-950 transition">
                                    <td class="px-6 py-4 text-sm font-medium text-white">{{ $customer->customer_code }}</td>
                                    <td class="px-6 py-4 text-sm text-slate-300">{{ $customer->user->name }}</td>
                                    <td class="px-6 py-4 text-sm text-slate-300">{{ $customer->phone_number }}</td>
                                    ...
                                    <td class="px-6 py-4 text-sm font-medium">
                                        <div class="flex gap-2">
                                            <a href="{{ route('admin.customers.show', $customer) }}" 
                                                class="text-amber-400 hover:text-amber-300">Lihat</a>
