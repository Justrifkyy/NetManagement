        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8 px-4 sm:px-0">
                <div class="bg-slate-900 rounded-xl shadow-sm border border-slate-800 p-6">
                    <h3 class="text-xl font-bold text-white mb-4">Master Area</h3>
                    ...
                    <div class="space-y-2 mb-6 max-h-64 overflow-y-auto">
                        @forelse ($masterAreas as $area)
                            <div class="flex justify-between items-center p-2 bg-slate-950 rounded">
                                <div>
                                    ...
                                    <p class="text-xs text-slate-300">{{ $area->code }}</p>
                                </div>
                        ...
                        @empty
                            <p class="text-slate-400 text-sm">Belum ada master area</p>
                        @endforelse
                    </div>
                    ...
                    <form action="{{ route('superadmin.master.storeArea') }}" method="POST" class="space-y-3">
                        ...
                        <input type="text" name="name" placeholder="Nama Area" required
                            class="w-full px-3 py-2 border border-slate-700 rounded-lg text-sm">
                ...
                <!-- Master Marketing -->
                <div class="bg-slate-900 rounded-xl shadow-sm border border-slate-800 p-6">
                    <h3 class="text-xl font-bold text-white mb-4">Master Marketing</h3>
                    ...
                    <div class="space-y-2 mb-6 max-h-64 overflow-y-auto">
                        @forelse ($masterMarketings as $marketing)
                            <div class="flex justify-between items-center p-2 bg-slate-950 rounded">
                                <div>
                                    ...
                                    <p class="text-xs text-slate-300">{{ $marketing->code }}</p>
                                </div>
                                <form action="#" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="text-red-600 hover:text-red-900 text-sm" onclick="return confirm('Hapus?')">Hapus</button>
                        ...
                        @empty
                            <p class="text-slate-400 text-sm">Belum ada master marketing</p>
                        @endforelse
                    </div>
            ...
            <div class="bg-slate-900 rounded-xl shadow-sm border border-slate-800 p-6 px-4 sm:px-0">
                <h3 class="text-xl font-bold text-white mb-4">Master Teknisi</h3>
                ...
                <div class="overflow-x-auto mb-6">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-4 py-6 text-center text-slate-400">Belum ada teknisi</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <form action="{{ route('superadmin.master.storeTechnician') }}" method="POST" class="grid grid-cols-1 md:grid-cols-4 gap-3">
                    ...
                    <input type="text" name="name" placeholder="Nama Teknisi" required
                        class="px-3 py-2 border border-slate-700 rounded-lg text-sm">
                    <input type="text" name="phone" placeholder="No. HP" required
                        class="px-3 py-2 border border-slate-700 rounded-lg text-sm">
