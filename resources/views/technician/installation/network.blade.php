        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="fade-in">
                    ...
                    <h1 class="text-4xl font-black text-white tracking-tighter">Konfigurasi <span class="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-300">Jaringan OLT</span></h1>
            ...
            @if ($errors->any())
                <div class="mb-8 p-5 bg-rose-500/10 border border-rose-500/20 rounded-[2rem] flex items-start gap-4 animate-shake">
                    ...
                    <div>
                        <p class="text-rose-400 font-black text-xs uppercase tracking-widest mb-2">Kendala Validasi Konfigurasi:</p>
            ...
            <form action="{{ route('technician.network.store', $installation->id) }}" method="POST" class="space-y-8">
                @csrf
                ...
                <div class="bg-slate-900/80 backdrop-blur-md rounded-[2.5rem] shadow-2xl border border-slate-800 overflow-hidden relative">
                    ...
                    <div class="bg-gradient-to-r from-indigo-600/20 to-transparent px-10 py-6 border-b border-slate-800/60 flex items-center gap-4">
                        ...
                        <h2 class="text-xl font-black text-white tracking-tight uppercase">Parameter Topologi & Interface</h2>
                    </div>

                    <div class="p-10 space-y-8">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="space-y-2">
                                <label class="block text-xs font-black text-slate-500 uppercase tracking-[0.2em] ml-1">Router Area Terdekat <span class="text-rose-500">*</span></label>
                                <div class="relative group">
                                    ...
                                    <input type="text" name="router_area" value="{{ old('router_area', $network->router_area) }}" required placeholder="Contoh: RT-01-MKS"
                            ...
                            <div class="space-y-2">
                                <label class="block text-xs font-black text-slate-500 uppercase tracking-[0.2em] ml-1">Port / Interface Output <span class="text-rose-500">*</span></label>
                                <div class="relative group">
                                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500 group-focus-within:text-indigo-400 transition-colors">
                                    ...
                                    <input type="text" name="port_interface" value="{{ old('port_interface', $network->port_interface) }}" required placeholder="Contoh: ether1 / sfp1"
                            ...
                            <div class="space-y-2">
                                <label class="block text-xs font-black text-slate-500 uppercase tracking-[0.2em] ml-1">VLAN ID <span class="text-slate-600 font-medium normal-case tracking-normal">(Opsional)</span></label>
                                <div class="relative group">
                                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500 group-focus-within:text-indigo-400 transition-colors">
                                    ...
                                    <input type="number" name="vlan_id" value="{{ old('vlan_id', $network->vlan_id) }}" placeholder="Contoh: 100"
                                        class="w-full pl-12 pr-4 py-4 bg-slate-800/50 border border-slate-700 text-white rounded-2xl focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all font-mono text-sm">
                                </div>
                            </div>

                            <div class="space-y-2">
                                <label class="block text-xs font-black text-slate-500 uppercase tracking-[0.2em] ml-1">Sumber OLT / Access Point <span class="text-rose-500">*</span></label>
                                <div class="relative group">
                                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500 group-focus-within:text-indigo-400 transition-colors">
                                    ...
                                    <input type="text" name="olt_access_point" value="{{ old('olt_access_point', $network->olt_access_point) }}" required placeholder="Masukkan ID OLT/AP"
                                        class="w-full pl-12 pr-4 py-4 bg-slate-800/50 border border-slate-700 text-white rounded-2xl focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all font-mono text-sm tracking-widest uppercase">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-4">
                            <label class="block text-xs font-black text-slate-500 uppercase tracking-[0.2em] ml-1">Mode Protokol Koneksi <span class="text-rose-500">*</span></label>
                            <div class="relative group">
                                <select name="connection_mode" required
                                    ...
                                    <option value="pppoe" @selected(old('connection_mode', $network->connection_mode) == 'pppoe') class="bg-slate-900 text-indigo-400">PPPoE (Point-to-Point Protocol)</option>
                                    <option value="static_ip" @selected(old('connection_mode', $network->connection_mode) == 'static_ip') class="bg-slate-900 text-emerald-400">Static IP (Alamat IP Tetap)</option>
                                    <option value="dhcp" @selected(old('connection_mode', $network->connection_mode) == 'dhcp') class="bg-slate-900 text-amber-400">DHCP (Dynamic IP)</option>
                                </select>
            ...
            <div class="mt-10 p-6 bg-indigo-500/5 border border-indigo-500/10 rounded-3xl flex items-start gap-4">
                ...
                <p class="text-[10px] text-slate-500 leading-relaxed font-black uppercase tracking-[0.2em]">
                    Standar Jaringan MGD: Pastikan nama Port/Interface dan VLAN ID (jika ada) sesuai dengan alokasi topologi jaringan Mikrotik/OLT pusat untuk mencegah bentrok IP (IP Conflict) dengan pelanggan lain.
                </p>
            </div>
        </div>
    </div>
